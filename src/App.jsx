...
// ── CGT 키워드 ────────────────────────────────────────────────────────────────
// isCGT()가 공백 무시 매칭을 하므로 "유전자치료" 하나로
// "유전자치료" / "유전자 치료" / "유전자 치료제" 모두 잡힘
const CGT_KW = [
  // 한국어 (공백 없는 형태로 통일 — 매칭 함수가 공백 무시)
  "세포치료","유전자치료","세포유전자","줄기세포","유전자편집","CAR-T","CRISPR",
  "NK세포","키메릭항원","항원수용체","아데노부속","렌티바이러스","CGT","iPSC",
  "TCR치료","TIL치료","항암바이러스","백시니아바이러스","TG-C",
  "조혈모세포이식","수지상세포","감마델타T","이중특이항체",
  "첨단바이오의약품","첨단재생의료","첨생법","재생의료",
  "전달벡터","바이러스벡터","유전자벡터",
  ...
  "hematopoietic","dendritic cell","ex vivo","in vivo CAR",
  "viral vector","HSV vector","herpes simplex vector","plasmid",
  "dystrophy","hemophilia","SMA","spinal muscular",
  // 기업명·브랜드 (글로벌이어도 기사 자체가 CGT면 수집)
  "LEO Pharma","Replicate","Krystal Bio","Avita","Passage Bio",
];
...
const GLOBAL_KW = [
  ...
  "LEO Pharma","Replicate Bioscience","Krystal Biotech","Passage Bio",
  "FDA","EMA","NIH","NCI",
];

// ── 카테고리 ──────────────────────────────────────────────────────────────────
const CATS = [
  { label: "임상",       dot: "#2563eb", keys: ["임상","phase","clinical","승인신청","ind ","first-in-human","투약 개시","환자 투약"] },
  { label: "허가",       dot: "#16a34a", keys: ["허가","approved","approval","fda 승인","식약처 허가","breakthrough","rmat","fast track","희귀의약품","품목허가"] },
  { label: "투자",       dot: "#7c3aed", keys: ["투자","유치","funding","series","ipo","억원","million","billion","deal","인수","협약","계약","라이선스"] },
  { label: "연구",       dot: "#d97706", keys: ["연구","결과","논문","발표","study","data","result","전임상","preclinical","효능","유효성","비임상"] },
  { label: "파이프라인", dot: "#dc2626", keys: ["파이프라인","pipeline","candidate","착수","도입","initiates","후보물질","파이프라인 확대"] },
  { label: "제품",       dot: "#0891b2", keys: ["제품","출시","상용화","product","launch","commercial","의료기기","device","급여","처방","판매 개시"] },
...
const CACHE_KEY = "cgt_v13";

// ── 유틸 ──────────────────────────────────────────────────────────────────────
// 공백 무시 매칭: "유전자치료" 키워드 하나로 "유전자 치료제"도 잡음
function isCGT(title, desc) {
  const s     = `${title} ${desc}`.toLowerCase();
  const sNoSp = s.replace(/\s+/g, "");
  return CGT_KW.some(k => {
    const kl     = k.toLowerCase();
    const klNoSp = kl.replace(/\s+/g, "");
    return s.includes(kl) || sNoSp.includes(klNoSp);
  });
}
...
function region(title, desc) {
  const s     = `${title} ${desc}`.toLowerCase();
  const sNoSp = s.replace(/\s+/g, "");
  const matchKw = kws => kws.some(k => {
    const kl = k.toLowerCase();
    return s.includes(kl) || sNoSp.includes(kl.replace(/\s+/g, ""));
  });
  if (matchKw(KOREAN_KW)) return "domestic";
  if (matchKw(GLOBAL_KW)) return "global";
...
// ── CORS 프록시 (3개 병렬 → 첫 성공 사용) ────────────────────────────────────
async function fetchXML(rssUrl) {
  const valid = t => typeof t === "string" && t.includes("<item");
...
async function fetchSource(src, log) {
  ...
  // 경로 2: rss2json 폴백 (count=100 요청)
  try {
    const r    = await fetch(
      `https://api.rss2json.com/v1/api.json?count=100&rss_url=${encodeURIComponent(src.url)}`,
      { cache: "no-store" }
    );
    ...
    log(src.name, `rss2json fail: ${e.message}`);
    return [];
  }
}
...
async function fetchAll(log) {
  ...
  // 30일 필터 (rss2json 10개 제한 대응 — 피드 자체가 최신순이라 문제없음)
  ...
  log("시스템", `수집 ${all.length}건 → 30일 내 ${recent.length}건`);
    ...
    <>
      ...
      >
        ...
        <td style={{
          ...
          fontSize: 11.5, color: "#b0a89a", fontFamily: "'IBM Plex Mono', monospace",
          ...
          <td colSpan={5} style={{ padding: 0 }}>
            <div style={{
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 18 }}>
                <span style={{
                  fontSize: 9.5, fontWeight: 700, color: "#fff", background: cat.dot,
              ...
              <p style={{
                margin: "0 0 24px", fontSize: 14.5, lineHeight: 2.05, color: "#28231f",
                fontFamily: "'Noto Serif KR', Georgia, serif",
                wordBreak: "keep-all", letterSpacing: "0.008em", fontWeight: 400,
              }}>
                {desc || "본문을 불러올 수 없어요."}
              </p>
              <a
        ...
        <div style={{ maxWidth: 960, margin: "0 auto", padding: "24px 40px" }}>
          ...
          <div style={{ borderLeft: "3px solid #1a1612", paddingLeft: 18 }}>
            ...
            <p style={{ margin: 0, fontSize: 12, color: "#b0a89a", fontFamily: "'IBM Plex Mono', monospace" }}>
              최근 30일 · 이슈 파급력 순 Top 10 · 국내·해외 자동 분류
            </p>
          </div>
        </div>
      </div>
