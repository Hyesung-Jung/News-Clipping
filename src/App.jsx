...
// 기존 소스별 RSS(10개 제한) 대신 Google News 통합 검색 사용
// → bosa.co.kr, biospectator.com 등 모든 한국 뉴스 사이트 기사를 한 번에 수집
const GN = "https://news.google.com/rss/search";
const gn  = (q, lang = "ko", cc = "KR") =>
  `${GN}?q=${encodeURIComponent(q)}&hl=${lang}&gl=${cc}&ceid=${cc}:${lang}`;

const SEARCHES = [
  // 국내 — CGT 핵심 용어
  { id: "국내-CGT",    url: gn("세포치료 OR 유전자치료 OR CAR-T OR CRISPR OR 줄기세포 OR iPSC OR AAV") },
  // 국내 — 기업명 1
  { id: "국내-기업A",  url: gn("큐로셀 OR 알지노믹스 OR 지씨셀 OR 박셀바이오 OR 코오롱생명과학 OR 툴젠 OR 메디포스트") },
  // 국내 — 기업명 2
  { id: "국내-기업B",  url: gn("엔케이맥스 OR 이뮨온시아 OR 유틸렉스 OR 오름테라퓨틱 OR 헬릭스미스 OR 제넥신 OR 차바이오텍") },
  // 국내 — 기업명 3
  { id: "국내-기업C",  url: gn("강스템바이오텍 OR 이엔셀 OR 바이오솔루션 OR 테고사이언스 OR 코아스템 OR 파미셀 OR 안트로젠") },
  // 국내 — 규제·정책
  { id: "국내-규제",   url: gn("첨생법 OR 첨단재생의료 OR 첨단바이오의약품 OR 세포치료제 허가 OR 유전자치료제 허가") },
  // 해외 — 글로벌 CGT
  { id: "글로벌-CGT",  url: gn("gene therapy OR cell therapy OR CAR-T OR AAV vector OR CRISPR", "en", "US") },
  // 해외 — 글로벌 기업
  { id: "글로벌-기업", url: gn("Novartis gene therapy OR Gilead CAR-T OR Bluebird OR UniQure OR Intellia OR Editas OR Vertex CRISPR", "en", "US") },
];
...
const CGT_KW = [
  ...
  "항암바이러스","백시니아","TG-C","조혈모","수지상세포","재생의료","첨생법",
  ...
  "viral vector","HSV vector","plasmid","dystrophy","hemophilia",
];
...
const KOREAN_KW = [
  ...
  "지아이이노베이션","다안바이오테라퓨틱스","다안바이오","에이비엘바이오",
  "차바이오텍","차바이오그룹","네이처셀",
  "이엔셀","강스템바이오텍","입셀","인투셀",
  "에스티팜","올리패스","셀리버리",
  "삼성바이오","삼성바이오로직스","바이넥스","셀트리온",
  "한미약품","종근당","대웅제약","유한양행",
  "JW중외제약","SK바이오팜","LG화학",
...
const CACHE_KEY = "cgt_v15";

// ── 유틸 ──────────────────────────────────────────────────────────────────────
function normSp(s) { return s.replace(/\s+/g, ""); }

function isCGT(title, desc) {
  const s = `${title} ${desc}`.toLowerCase();
  const n = normSp(s);
  return CGT_KW.some(k => { const kl = k.toLowerCase(); return s.includes(kl) || n.includes(normSp(kl)); });
}

function region(title, desc) {
  const s = `${title} ${desc}`.toLowerCase();
  const n = normSp(s);
  const hit = kws => kws.some(k => { const kl = k.toLowerCase(); return s.includes(kl) || n.includes(normSp(kl)); });
...
// ── Google News RSS 파싱 ──────────────────────────────────────────────────────
function parseGNews(text) {
  const xml = new DOMParser().parseFromString(text, "text/xml");
  if (xml.querySelector("parsererror")) throw new Error("xml parse error");
  return [...xml.querySelectorAll("item")].flatMap(el => {
    const g   = tag => el.querySelector(tag)?.textContent?.trim() ?? "";
    const title  = g("title").replace(/\s*-\s*[^-]+$/, "").trim(); // "제목 - 매체명" → "제목"
    const src    = el.querySelector("source")?.textContent?.trim() || g("title").match(/-\s*([^-]+)$/)?.[1]?.trim() || "기타";
    const link   = g("link") || g("guid") || "#";
    const date   = g("pubDate") || "";
    const desc   = stripHTML(g("description") || "");
    if (!isCGT(title, desc + " " + title)) return [];
    return [{ title, link, date, source: src, desc }];
  });
}

// ── CORS 프록시 병렬 시도 ─────────────────────────────────────────────────────
async function fetchXML(url) {
  const valid = t => typeof t === "string" && (t.includes("<item") || t.includes("<entry"));
  const via   = async fn => { const t = await fn(); if (!valid(t)) throw new Error("empty"); return t; };
  return Promise.any([
    via(() =>
...
// ── 단일 검색 실행 ────────────────────────────────────────────────────────────
async function runSearch(search, log) {
  try {
    const text  = await fetchXML(search.url);
    const items = parseGNews(text);
    log(search.id, `OK · ${items.length}건`);
    return items;
  } catch (e) {
    log(search.id, `fail: ${e.message}`);
    return [];
  }
}
...
async function fetchAll(log) {
  const results = await Promise.allSettled(SEARCHES.map(s => runSearch(s, log)));
  const all     = [...new Map(
    results.flatMap(r => r.status === "fulfilled" ? r.value : []).map(it => [it.link, it])
  ).values()]; // 링크 기준 중복 제거

  const monthAgo = Date.now() - 7 * 24 * 60 * 60 * 1000;
  const recent   = all.filter(it => { const t = new Date(it.date).getTime(); return !isNaN(t) && t >= monthAgo; });
  log("시스템", `수집 ${all.length}건 → 30일 내 ${recent.length}건 (중복 제거 후)`);
...
function Row({ item, idx, isLast }) {
  ...
  return (
    <>
      <tr onClick={() => setOpen(o => !o)} style={{ borderBottom: isLast && !open ? "none" : "1px solid #f0ede8", cursor: "pointer", background: open ? "#fafaf8" : "transparent", transition: "background .15s", animation: `fi .3s ease ${idx * 0.04}s both` }}
      ...
      {open && <div style={{ padding: "12px 18px", background: "#fff", maxHeight: 240, overflowY: "auto" }}>
        {logs.map((l, i) => (
          <div key={i} style={{ display: "flex", gap: 12, padding: "4px 0", borderBottom: "1px solid #f5f3ef", fontFamily: "'IBM Plex Mono', monospace", fontSize: 10.5 }}>
            <span style={{ color: "#c0b8ae", flexShrink: 0, minWidth: 120 }}>{l.src}</span>
...
export default function App() {
  ...
  const load = useCallback(async (force = false) => {
    ...
    if (!force) {
      const c = loadCache();
      if (c) { setDomestic(c.domestic); setGlobal(c.global); setUpdated(new Date(c.fetchedAt)); setFromCache(true); setLoading(false); return; }
    }
    setFromCache(false);
      ...
      <div style={{ borderBottom: "1px solid #ede9e3", background: "#fff" }}>
        <div style={{ maxWidth: 960, margin: "0 auto", padding: "24px 40px" }}>
          ...
          <div style={{ borderLeft: "3px solid #1a1612", paddingLeft: 18 }}>
            ...
            <p style={{ margin: 0, fontSize: 12, color: "#b0a89a", fontFamily: "'IBM Plex Mono', monospace" }}>최근 7일 · Google News 통합 검색 · 국내·해외 자동 분류</p>
          </div>
        </div>
      </div>
