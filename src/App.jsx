...
function region(title, desc) {
  ...
  const score = (kws, w) => kws.reduce((sum, k) => {
    const kl = k.toLowerCase();
    return sum + (s.includes(kl) || n.includes(noSp(kl)) ? w : 0);
  }, 0);

  const domesticScore =
    score(KR_COMPANIES,  3) +
    score(KR_AGENCIES,   2) +
    score(KR_INDICATORS, 1);

  const globalScore =
    score(GL_COMPANIES, 3) +
    score(GL_AGENCIES,  1);

  if (domesticScore === 0 && globalScore === 0) {
...
async function fetchNaverQuery(query, log) {
  try {
    const r = await fetch(`/api/naver?query=${encodeURIComponent(query)}&display=100&sort=date`);
    if (!r.ok) throw new Error(`HTTP ${r.status}`);
    const data = await r.json();
    if (!data.items) throw new Error(data.errorMessage || "no items");
    const weekAgo = Date.now() - WEEK_MS;
    const items = data.items.flatMap(it => {
      const title = stripHTML(it.title), desc = stripHTML(it.description);
...
function Row({ item, idx, isLast, isOpen, onToggle }) {
  ...
  const [refinedCat,  setRefinedCat]  = useState(null); // 크롤링 후 재분류 카테고리

  // 테이블 행 카테고리: 스니펫 기준 (항상 표시)
  const rowCat = classify(item.title, item.desc);
  // 펼침 영역 카테고리: 크롤링 본문 기준 재분류 (있으면 우선)
  const cat = refinedCat || rowCat;
  ...
  const handleClick = async () => {
    ...
    if (!isOpen && fullText === null && !crawling) {
      ...
      if (resolved.length > 100) {
        setRefinedCat(classify(item.title, resolved));
      }
      setCrawling(false);
    }
  };
      ...
      >
        <td style={{ padding:"14px 18px",width:90,whiteSpace:"nowrap" }}>
          <span style={{ display:"inline-flex",alignItems:"center",gap:6 }}>
            <span style={{ width:5,height:5,borderRadius:"50%",background:rowCat.dot,flexShrink:0 }}/>
            <span style={{ fontSize:11,color:rowCat.dot,fontWeight:600,letterSpacing:"0.03em" }}>{rowCat.label}</span>
          </span>
        </td>
        ...
        <tr style={{ borderBottom: isLast ? "none" : "1px solid #f0ede8" }}>
          <td colSpan={5} style={{ padding:0 }}>
            <div style={{ padding:"24px 32px 24px 86px",background:"linear-gradient(160deg,#faf9f7 0%,#f5f3ef 100%)",borderLeft:`3px solid ${cat.dot}`,animation:"ed .2s ease" }}>
              ...
              {/* 본문 — 크롤링 완료 시 fadeIn으로 자연스럽게 전환, 문단 분리 렌더링 */}
              <div
                key={contentKey}
                style={{
                  marginBottom: 20,
                  animation: contentKey === "full" ? "fadeIn .45s ease" : "none",
                  // 8줄 clamp — 스니펫(로딩 중)일 때만 적용
                  ...(contentKey !== "full" ? {
                    display: "-webkit-box",
                    WebkitLineClamp: 8,
              ...
              >
                {paragraphs.map((para, i) => (
                  <p key={i} style={{ ...pStyle, marginBottom: i < paragraphs.length - 1 ? 12 : 0 }}>
                    {para}
                  </p>
                ))}
              </div>
