  ...
  return (
    <>
      <tr
        ...
        style={{
          borderBottom: isLast && !isOpen ? "none" : "1px solid #f0ede8",
          cursor: "pointer",
          background: isDragOver ? "#eff6ff" : isOpen ? "#fafaf8" : hovered ? "#fdfcfb" : "transparent",
          borderTop: isDragOver ? "2px solid #2563eb" : undefined,
          transition: "background .15s",
          animation: `fi .3s ease ${idx*.04}s both`,
        }}
      >
        ...
        {/* 마우스 오버 시: 액션 버튼 / 기본: 펼침 토글 */}
        <td style={{ padding:"0 10px",width:96,whiteSpace:"nowrap",textAlign:"right" }}>
          {hovered ? (
            <span data-action style={{ display:"inline-flex",alignItems:"center",gap:4 }}>
              <span style={{ color:"#c0b8ae",fontSize:14,cursor:"grab",userSelect:"none",padding:"0 2px" }} title="드래그로 순서 변경">⠿</span>
              <button data-action onClick={e=>{e.stopPropagation();onHide();}}
                title="목록에서 제거" style={{ border:"none",background:"none",cursor:"pointer",color:"#dc2626",fontSize:15,padding:"1px 4px",lineHeight:1,borderRadius:3 }}>×</button>
              <button data-action onClick={e=>{e.stopPropagation();onMove();}}
                title={sectionName==="domestic"?"해외 뉴스로 이동":"국내 뉴스로 이동"}
                style={{ border:"1px solid #e0dbd4",background:"#fff",cursor:"pointer",color:"#6b5f54",fontSize:9,padding:"2px 6px",borderRadius:3,fontFamily:"'IBM Plex Mono',monospace",lineHeight:1.4,whiteSpace:"nowrap" }}>
                {sectionName==="domestic"?"→해외":"←국내"}
              </button>
            </span>
          ) : (
            <span style={{ fontSize:9,color:"#d0c8be" }}>{isOpen?"▲":"▼"}</span>
          )}
        </td>
      </tr>
        ...
        <table style={{ width:"100%",borderCollapse:"collapse" }}>
          <thead>
            <tr style={{ background:"#faf9f6",borderBottom:"1px solid #ede9e3" }}>
              {[["구분",90],["일자",68],["헤드라인",null],["출처",160],["",96]].map(([h,w])=>(
              ...
              <div style={{ display:"flex",alignItems:"center",gap:16 }}>
                ...
                {((edits.hidden||[]).length > 0 || Object.keys(edits.moved||{}).length > 0 || (edits.domOrder||[]).length > 0 || (edits.gloOrder||[]).length > 0) && (
                  <button onClick={onReset}
                    style={{ background:"#fff7ed",color:"#c2410c",border:"1px solid #fed7aa",borderRadius:4,padding:"5px 14px",fontSize:10.5,fontWeight:600,fontFamily:"'IBM Plex Mono',monospace",letterSpacing:"0.06em",cursor:"pointer",transition:"all .2s" }}
                    title="모든 편집(숨김·이동·순서) 초기화">
                    초기화
                  </button>
                )}
              </div>
