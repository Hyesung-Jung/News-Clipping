...
// ── 네이버 뉴스 검색 쿼리 ─────────────────────────────────────────────────────
// /api/naver?query=... → Vercel 서버리스 함수 경유 (CORS 우회)
// 쿼리당 최대 100건 · 하루 25,000회 무료
const NAVER_QUERIES = [
  // 국내 — CGT 핵심 용어
  "세포치료 유전자치료",
  "CAR-T CRISPR",
...
function getDesc(item) {
  ...
  const cut = raw.slice(0, 1100), last = Math.max(cut.lastIndexOf("다. "), cut.lastIndexOf(". "));
...
function srcFromUrl(url) {
  try {
    ...
    return SOURCE_MAP[host] || host.split(".")[0];
  } catch { return "기타"; }
}
...
async function fetchAll(log) {
  const results = await Promise.allSettled(NAVER_QUERIES.map(q => fetchNaverQuery(q, log)));
  const all     = results.flatMap(r => r.status === "fulfilled" ? r.value : []);

  // 링크 기준 중복 제거
  const deduped = [...new Map(all.map(i => [i.link, i])).values()];
  deduped.forEach(it => { it.tokens = tokenize(it.title); });

  log("시스템", `수집 ${all.length}건 → 중복 제거 후 ${deduped.length}건`);
...
// ── 캐시 ──────────────────────────────────────────────────────────────────────
function loadCache() {
  try { const r = localStorage.getItem(CACHE_KEY); if (!r) return null; const { date, data } = JSON.parse(r); return date === todayStr() ? data : null; } catch { return null; }
}
function saveCache(d) {
  try { localStorage.setItem(CACHE_KEY, JSON.stringify({ date: todayStr(), data: d })); } catch {}
}
    ...
    <>
      ...
      >
        ...
        <td style={{ padding: "14px 18px", width: 160, fontSize: 11, color: "#c0b8ae", whiteSpace: "nowrap", textAlign: "right" }}>
          ...
          {item.sourceCount > 1 && (
            <span style={{ marginLeft: 6, padding: "1px 6px", background: "#fef3c7", color: "#92400e", borderRadius: 3, fontSize: 9.5, fontWeight: 700, fontFamily: "'IBM Plex Mono', monospace", border: "1px solid #fde68a" }}>
              +{item.sourceCount - 1}
            </span>
          )}
        </td>
    ...
    <div style={{ marginBottom: 44 }}>
      ...
      <div style={{ border: "1px solid #ede9e3", borderRadius: 8, overflow: "hidden" }}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          ...
          <tbody>
            {loading && (
              <tr><td colSpan={5} style={{ padding: "52px 0", textAlign: "center", background: "#fff" }}>
            ...
            {!loading && items.length === 0 && (
              <tr><td colSpan={5} style={{ padding: "44px 0", textAlign: "center", color: "#c0b8ae", fontSize: 11, background: "#fff", fontFamily: "'IBM Plex Mono', monospace" }}>no results found</td></tr>
            )}
