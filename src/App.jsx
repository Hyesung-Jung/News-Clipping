...
const CATS = [
  ...
  { label: "투자",       dot: "#7c3aed", keys: ["투자","유치","funding","series","ipo","억원","million","billion","deal","인수합병","m&a"] },
  { label: "연구",       dot: "#d97706", keys: ["연구","결과","논문","발표","study","data","result","preclinical","전임상"] },
...
const CACHE_KEY = "cgt_v8";
// [핵심 수정] count=100 파라미터 추가 + corsproxy 직접 파싱으로 피드 전량 수집
const RSS_API   = "https://api.rss2json.com/v1/api.json?count=100&rss_url=";
...
function isGlobal(item) { return classifyRegion(item) === "global"; }
...
// ── RSS 수집: corsproxy 직접 XML 파싱 (전체 피드) + rss2json 폴백 ────────────
// 기존 rss2json 무료플랜은 피드당 최대 10개 제한 → 7일 내 기사가 적게 잡히는 원인
// corsproxy로 직접 파싱하면 피드 전량(수십~수백 건) 수집 가능
async function fetchSourceDirect(src) {
  ...
  return [...xml.querySelectorAll("item")].map(item => {
    ...
    const title = g("title");
    const desc  = g("description") || g("encoded") || g("summary") || "";
    const link  = g("link") || g("guid") || "#";
    const date  = g("pubDate") || g("date") || g("updated") || "";
    if (!isCGT({ title, description: desc })) return null;
    return { title, link, date, source: src.name, desc: stripHTML(desc) };
  }).filter(Boolean);
}

async function fetchSourceFallback(src) {
  const r = await fetch(`${RSS_API}${encodeURIComponent(src.url)}`);
  const j = await r.json();
  if (j.status !== "ok") return [];
  return (j.items ?? [])
    .filter(it => isCGT({ title: it.title ?? "", description: it.description ?? "" }))
    .map(it => ({
      title:  (it.title ?? "").trim(),
...
async function fetchSource(src) {
  try {
    const res = await fetchSourceDirect(src);
    if (res.length > 0) return res;
    return await fetchSourceFallback(src);
  } catch {
    try { return await fetchSourceFallback(src); } catch { return []; }
  }
}
...
function clusterStories(items) {
  ...
  for (const item of items) {
    ...
    for (const cluster of clusters) {
      if (cluster.some(c => isSameStory(c, item))) { cluster.push(item); added = true; break; }
    }
  ...
  return (
    <>
      ...
      >
        ...
        {/* 헤드라인 */}
        <td style={{ padding: "14px 18px", fontSize: 13.5, color: "#1a1612", fontWeight: 500, lineHeight: 1.5 }}>
          {item.title}
        </td>
        ...
        <tr style={{ borderBottom: isLast ? "none" : "1px solid #f0ede8" }}>
          <td colSpan={5} style={{ padding: 0 }}>
            <div style={{
              ...
              background: "linear-gradient(160deg, #faf9f7 0%, #f6f4f0 100%)",
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 18 }}>
                ...
                <span style={{ fontSize: 10, color: "#c8c0b8", fontFamily: "'IBM Plex Mono', monospace" }}>
                  {fmtDate(item.date)}
                </span>
                {item.sourceCount > 1 && (
              ...
              <a
                ...
                style={{
                  ...
                  border: `1px solid ${cat.dot}60`, borderRadius: 4,
                  fontFamily: "'IBM Plex Mono', monospace",
      ...
      <div style={{ border: "1px solid #ede9e3", borderRadius: 8, overflow: "hidden" }}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          ...
          <tbody>
            {loading && (
              <tr>
                <td colSpan={5} style={{ padding: "48px 0", textAlign: "center", background: "#fff" }}>
